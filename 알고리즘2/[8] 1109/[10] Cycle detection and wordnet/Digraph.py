...
def sap(g, aList, bList):
    ...
    while not Q.empty():
        ...
        if v[2] >= sapLength: 
            break
        for w in g.adj[v[0]]:
            temp_visited = [(x[0],x[1]) for x in visited]
            if v[1] == 'a':
                if (w,'a') not in temp_visited:
                    visited.append((w,'a',v[2]+1))
                    Q.put((w, 'a', v[2]+1))
                for i, vertex in enumerate(temp_visited):
                    if (w,'b') == vertex:
                        if sapLength > v[2] + 1 + visited[i][2]:
                            sapLength = v[2] + 1 + visited[i][2]
                            sapVertex = w
            elif v[1] == 'b':
                if (w,'b') not in temp_visited:
                    visited.append((w,'b', v[2]+1))
                    Q.put((w, 'b', v[2]+1))
                for i, vertex in enumerate(temp_visited):
                    if (w,'a') == vertex:
                        if sapLength > v[2] + 1 + visited[i][2]:
