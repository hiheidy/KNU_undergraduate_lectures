...
def solveManhattan(initialBoard):
    ...
    solvQue = queue.PriorityQueue()
    
    # 초기 상태 큐에 넣기
    solvQue.put((0+initialBoard.manhattan(), initialBoard, 0, None))
    ...
    while True: 
        ...
        move = minNode[0] - minNode[1].manhattan()
        for neighbor in neighbors:
            if minNode[3] == None:                      # 직전 상태가 없을 때
                solvQue.put((move + 1 + neighbor.manhattan(), neighbor, move + 1, minNode))
            elif neighbor != minNode[3][1]:             # 직전 상태일 때 제외
                solvQue.put((move + 1 + neighbor.manhattan(), neighbor, move + 1, minNode))
    
    # 마지막 상태를 기점으로 거슬러 올라가면서 저장
    while minNode != None:
