...
class SCC:
    def __init__(self, g): # Do strongly-connected-components pre-processing, based on Kosaraju-Sharir algorithm
        ...
        self.count = 0
        rev_g = g.reverse()
        topological_order = topologicalSort(rev_g)
        
        for i in range (len(topological_order)):
            if self.id[topological_order[i]] == -1 :
                dfs_result = DFS(self.g, topological_order[i])
                for j in range(len(topological_order)):
                    if self.id[topological_order[j]] == -1:  # if not visited
                        if dfs_result.hasPathTo(topological_order[j]):
                            self.id[topological_order[j]] = self.count
                self.count += 1
